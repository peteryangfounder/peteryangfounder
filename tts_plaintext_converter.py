...
def _process_projection(name: str, s: str, i: int) -> Tuple[str, int]:
    ...
    if onto and subject:
        return f"perpendicular of {subject} onto {onto}", i
    if subject:
        return f"perpendicular of {subject}", i
    if onto:
        return f"perpendicular onto {onto}", i
...
def tex_to_words(s: str) -> str:
    ...
    text = re.sub(r"absolute value bars\s+absolute value bars\s+(.+?)\s+absolute value bars\s+absolute value bars", lambda m: "norm of " + m.group(1), text)
